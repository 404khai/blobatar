 ...
 * avatar wears a given expression at the same strength.
 */
...
describe("the contract", () => {
  test("idle is byte-identical to no expression at all", () => {
    ...
    // must not move a single existing avatar.
    for (const s of SEEDS.slice(0, 500)) {
      expect(avatar(s, { expression: idle })).toBe(avatar(s));
    }
  });
  ...
  test("a named expression changes the markup", () => {
    for (const [name, e] of NAMED) {
      expect(avatar("seed-1", { expression: e }), name).not.toBe(
        avatar("seed-1"),
      );
    }
  });
  ...
  test("character ignores expressions rather than half-applying them", () => {
    ...
    for (const [name, e] of ALL) {
      expect(
        avatar("seed-1", { variant: "character", expression: e }),
        name,
      ).toBe(avatar("seed-1", { variant: "character" }));
    }
  });
  ...
  test("nothing leaves the frame, including the body offset", () => {
    ...
    for (const [name, e] of NAMED) {
      for (const s of SEEDS.slice(0, 1500)) {
        const svg = avatar(s, { expression: e, background: false });
...
describe("the tilt has real headroom", () => {
  ...
  test("every avatar wears the same strength of a given expression", () => {
    ...
    for (const [name, e] of NAMED) {
      const tilts = new Set(
        SEEDS.slice(0, 800).map((s) => {
          const l = _layout(s, { expression: e });
          const idle = _layout(s);
          return (
            Math.round(
              ((l.eyes[0] as { rot: number }).rot -
                (idle.eyes[0] as { rot: number }).rot) *
                1e6,
            ) / 1e6
          );
        }),
