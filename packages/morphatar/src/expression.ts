/**
 * Expressions — `morphatar/expression`. See docs/expression-spec.md.
 *
 ...
 * gated on hover, an expression is triggered and gated on nothing. An avatar can
 * be sad and still breathing.
 *
 * Pose-only, by definition — every channel below moves a part the avatar already
 ...
 *
 * **Each expression is passed in, not named.** `avatar()` never imports this
 ...
 * documented on `_parts` in `avatar.ts`.
 */
 ...
 * is the one move that makes an avatar stop looking like itself.
 *
  ...
  return {
    l: {
      ...
      eyes: l.eyes.map((e, i) => ({
        ...
        rot: e.rot + (p.tilt + (i ? p.tilt2 : 0)) * (i ? 1 : -1),
      })),
    },
