 ...
 * are actually on screen, not from the ramp that produced them: `AvatarOptions.palette`
 * lets a consumer override `head` or `eye` outright, and a hot pair derived from
 * the ramp instead would tint toward a color the avatar never wore.
 */
 ...
 * Travelling the whole way is the opposite failure: every avatar in the roster
 ...
 * pale avatar recognisably pale and an ink one recognisably dark while giving
