 ...
 *
 * `"hover"` animates one avatar at a time, which is both the aesthetic answer
 * (ambient motion seen constantly is motion worth removing) and the performance
 * one. `"always"` is the escape hatch for the single-avatar case — a profile
 ...
 * between their two periods but locks every avatar into the *same* drift, which
 * is the unison problem again, one level up.
 *
 * These keys cost nothing in compatibility: traits are string-addressed, so
 * adding `motion.*` cannot perturb any existing avatar.
 */
export function motionVars(t: Traits): Record<string, string> {
  ...
  return {
    ...
    // Where this avatar looks when it glances. One shared `@keyframes` visits
    // the same *sequence* of fixations on every avatar, so without a per-seed
