...
declare global {
  ...
  var CASES: {
    ...
    name: string;
    lean: number;
    /** Serialized `avatar()` output — the pose baked into path data. */
    static: string;
    /** The same avatar animated: no pose in the markup, all of it in `vars`. */
    cls: string;
    inner: string;
...
async function checkContinuity() {
  ...
  function Harness() {
    ...
    setExpr = set;
    return <Avatar seed="alain00" animate="always" expression={e} size={200} />;
  }
  ...
  // restarted animation throws it away — every reacting avatar snaps into phase
...
async function checkPacing() {
  ...
  function Harness() {
    const [e, set] = useState<typeof happy | undefined>(undefined);
    setExpr = set;
    return <Avatar seed="alain00" animate="always" expression={e} size={SIZE} />;
  }
  createRoot(host).render(<Harness />);
  await frame();
  await frame();
 ...
 * identity at amplitude zero and left every avatar in the grid quivering, would
 * both pass every other check in this file.
 *
 * So both halves are measured, on the same element, in screen coordinates: the
 * angry avatar moves, and the happy one does not move at all.
 */
async function checkShake() {
  ...
  createRoot(host).render(
    <>
      <Avatar seed="alain00" animate="always" expression={mad} size={SIZE} />
      <Avatar seed="alain00" animate="always" expression={happy} size={SIZE} />
    </>,
  );
  await frame();
  await frame();
  ...
  // on the same phase twice and report a stationary avatar.
  ...
  report(
    "D the tremor moves an angry avatar and nothing else",
    angry > 1 && calm < 0.01,
 ...
 * wrong as the *only* evidence the curve runs: a live avatar has five idle
...
(async () => {
  ...
  // The driver polls this rather than the DOM: it runs the page over CDP, so a
  // plain global is the whole handoff.
