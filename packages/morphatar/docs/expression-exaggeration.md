...
- **Determinism.** Existing avatars must not move. `idle` stays byte-identical
...
avatars; most avatars are 32px in a list. Check both before signing off.
...
- In `blob` the silhouette is the identity. Deforming it per-expression spends
  the thing that distinguishes avatars to say something the eyes say better.
