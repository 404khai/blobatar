...
Status: in use — the fixture is generated by `tools/export-reference-vectors.ts`
against blobatar `2.4.0` and checked read-only by
`packages/flutter/test/dart/parity_test.dart`
Reference: Blobatar `2.4.0`, generation 2  
Companion: [`phase-0-coordination.md`](./phase-0-coordination.md)

The reference vectors exist as a published artifact in this repository so any
port has one definition of correct. This document fixes the strategy, schema,
coverage, and comparison rules used by the Dart implementation.
