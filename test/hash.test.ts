...
describe("determinism", () => {
  test("same seed produces identical output", () => {
    expect(avatar("alain")).toBe(avatar("alain"));
  });

  test("different seeds produce different output", () => {
    expect(avatar("alain")).not.toBe(avatar("bob"));
  });
});

describe("normalization", () => {
  test("case, whitespace and NFC form are equivalent", () => {
    const base = avatar("Alain@Example.com");
    expect(avatar("alain@example.com")).toBe(base);
    expect(avatar("  ALAIN@EXAMPLE.COM  ")).toBe(base);
  });

  test("decomposed and precomposed accents agree", () => {
    // "café": U+00E9 vs "e" + U+0301
    expect(avatar("café")).toBe(avatar("café"));
    expect(normalizeSeed("café")).toBe("café");
  });

  test("normalize: false hashes the raw string", () => {
    expect(avatar("Alain", { normalize: false })).not.toBe(avatar("alain", { normalize: false }));
  });
});
